...
Racional Racional::operator+(const Racional &rac) const{
    double num = rac.denom_ * num_ + denom_ * rac.num_;
    double denom = denom_ * rac.denom_;
...
Racional Racional::operator*(const Racional &rac) const{
    double num = num_ * rac.num_;
    double denom = denom_ * rac.denom_;
    return Racional(num, denom);
}
Racional Racional::operator/(const Racional &rac) const{
    if(denom_ == 0 || rac.denom_ == 0) throw std::runtime_error("Math error in class Complejo2, method 'operator/' operation - Attempted to divide by 0");
    double num = num_ * rac.denom_;
    double denom = denom_ * rac.num_;
    return Racional(num, denom);
}
...
std::ostream& operator<<(std::ostream &stream_out, const Racional &obj){ 
    double rac1_num = obj.num_;
    double rac1_denom = obj.denom_;
    std::map<std::string, int> test;
    rac1_num > rac1_denom ? test["Greater"] = rac1_num : test["Greater"] = rac1_denom;
    while(fmod(rac1_num, 10) == 0 && fmod(rac1_denom, 10) == 0){
        rac1_num /= 10;
        rac1_denom /= 10;
    }
    for (double i = test["Greater"]; i > 1; i--) {
        if ((fmod(obj.denom_, i) == 0) && (fmod(rac1_num, i) == 0)){ 
            std::cout << "fmod(" << rac1_num << "," << i << ") == 0)" << " == " << "(fmod(" << rac1_denom << "," << i << ") == 0  ->  num_ = " << rac1_num / i << " | " << "denom_ = " << rac1_denom / i << std::endl; 
            rac1_num /= i;  
            rac1_denom /= i;  
        } else std::cout << "fmod(" << rac1_num << "," << i << ") == 0)" << " != " << "(fmod(" << rac1_denom << "," << i << ") == 0" << std::endl;
    }
